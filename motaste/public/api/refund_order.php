...
try {
    ...
    if (!$result['success']) {
        ...
        echo json_encode(['success' => false, 'error' => $result['error'] ?? 'Unable to refund order']);
        exit;
    }
    ...
    DB::table('order_activity_logs')->insert([
        ...
        'summary' => 'Order refunded and stock restored',
        'details' => json_encode([
    ...
    logApiEvent('order_refunded', ['order_id' => $orderId, 'order_number' => $result['orderNumber'] ?? null]);
    ...
    echo json_encode(['success' => false, 'error' => 'Unable to refund order', 'details' => apiErrorDetail($error)]);
}

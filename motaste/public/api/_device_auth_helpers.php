...
/**
 * Global "remember trusted devices" toggle (managed by the Admin from the
 * Credentials section). Stored in the key/value app_settings table.
 *
 *   - ENABLED (default): recognized devices skip the emailed verification
 *     code; only first-time devices are challenged.
 *   - DISABLED: EVERY login requires the emailed verification code, even from
 *     a previously trusted device, until the Admin turns the feature back on.
 */
...
/**
 * Whether the Admin has the trust-device feature enabled. Defaults to enabled
 * so existing deployments keep their current behavior until the toggle is used.
 */
function isTrustDeviceEnabled(): bool
{
    ensureAppSettingsTable();

    try {
        $value = DB::table('app_settings')->where('key', 'trust_device_enabled')->value('value');
        if ($value === null) {
            return true;
        }
        return filter_var($value, FILTER_VALIDATE_BOOLEAN);
    } catch (Throwable $error) {
        // Fail open to the existing behavior rather than locking everyone out.
        error_log('isTrustDeviceEnabled failed: ' . $error->getMessage());
        return true;
    }
}

/**
 * Persist the trust-device toggle (admin-only endpoint).
 */
function setTrustDeviceEnabled(bool $enabled): void
{
    ensureAppSettingsTable();

    DB::table('app_settings')->updateOrInsert(
        ['key' => 'trust_device_enabled'],
        [
            'value' => $enabled ? '1' : '0',
            'updated_at' => now()->toDateTimeString(),
            'created_at' => now()->toDateTimeString(),
